...
def render_quick_convert_mode():
    ...
    # --- FIX: เอา Header ออกแล้ว ---
    # st.markdown("## ⚡ แก้ PDF เพี้ยนเป็น Word (Quick Fix)")  <-- ลบทิ้ง
    # st.caption("...") <-- ลบทิ้ง
    # -----------------------------

    # 1. Global Settings (Expander)
    with st.expander("⚙️ ตั้งค่า (Settings & Upload)", expanded=True):
        ...
        # 2. Upload Zone (ย้ายมาไว้ใน Expander ให้เหมือนหน้า OCR)
    ...
    if uploaded_file and api_key and selected_model:
        ...
        # 3. Selection Tabs (อยู่นอก Expander)
    ...
    # 3. Download Button (อยู่นอก Expander)
